...
<<<<<<< HEAD
# 프로젝트 루트 디렉토리 설정
PROJECT_ROOT="/Users/sungmincho/pocker_v3"
=======
...
<<<<<<< HEAD
cd "$PROJECT_ROOT/server" && nohup npm run dev:fg > "$PROJECT_ROOT/logs/server.log" 2>&1 &
=======
...
<<<<<<< HEAD
cd "$PROJECT_ROOT/client" && nohup npm run dev > "$PROJECT_ROOT/logs/client.log" 2>&1 &
=======
...
echo "📊 포트 상태 확인:"
<<<<<<< HEAD
lsof -i :9000 | grep LISTEN && echo "✅ 서버가 포트 9000에서 실행 중" || echo "❌ 서버 시작 실패"
lsof -i :4000 | grep LISTEN && echo "✅ 클라이언트가 포트 4000에서 실행 중" || echo "❌ 클라이언트 시작 실패"
=======
